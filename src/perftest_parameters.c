...
static const char* portStates[] = {"Nop", "Down", "Init", "Armed", "",
                                   "Active Defer"};
...
static int parse_mac_from_str(char* mac, u_int8_t* addr) {
    ...
    while (tmpField != NULL && fieldNum < MAC_ARR_LEN) {
        ...
        if (*chk != 0) {
            fprintf(stderr,
                    "Non-digit character %c (%0x) detected in field %d\n", *chk,
                    *chk, fieldNum);
            return FAILURE;
        }
 ...
 ******************************************************************************/
static void
usage(const char* argv0, VerbType verb, TestType tst, int connection_type) {
    printf("Usage:\n");
    ...
    printf(" Number of exchanges (at least %d, default %d)\n", MIN_ITER,
            ((verb == WRITE) && (tst == BW)) ? DEF_ITERS_WB : DEF_ITERS);
...
static void init_perftest_params(struct perftest_parameters* user_param) {
    ...
    user_param->tx_depth = (user_param->tst == BW ||
                            user_param->tst == LAT_BY_BW) ? DEF_TX_BW
                                                          : DEF_TX_LAT;
    user_param->qp_timeout = DEF_QP_TIME;
    user_param->test_method = RUN_REGULAR;
    user_param->cpu_freq_f = OFF;
    user_param->connection_type = (user_param->connection_type == RawEth)
                                  ? RawEth : RC;
    ...
    user_param->iters = (user_param->tst == BW && user_param->verb == WRITE)
...
static void change_conn_type(int* cptr, VerbType verb, const char* optarg) {
    ...
    if (strcmp(connStr[0], optarg) == 0) {
        ...
        *cptr = UC;
        if (verb == READ || verb == ATOMIC) {
            fprintf(stderr,
                    " UC connection not possible in READ/ATOMIC verbs\n");
            exit(1);
        }
        ...
        *cptr = DC;
        ...
        fprintf(stderr,
                " Invalid Connection type . please choose from {RC,UC,UD}\n");
        exit(1);
    }
}
...
int set_eth_mtu(struct perftest_parameters* user_param) {
    ...
    if (user_param->mtu >= MIN_MTU_RAW_ETERNET &&
        user_param->mtu <= MAX_MTU_RAW_ETERNET) {
...
void flow_rules_force_dependecies(struct perftest_parameters* user_param) {
    ...
    if (user_param->flows != DEF_FLOWS) {
        if (user_param->is_server_port == OFF) {
            fprintf(stderr,
                    " Flows feature works with UDP/TCP packets only for now\n");
            exit(1);
        }
        if (user_param->test_type == ITERATIONS) {
            ...
            if (user_param->iters / min_iter_req < 1) {
                fprintf(stderr,
                        " Current iteration number will not complete full cycle on all flows, it need to be multiple of the product between flows and flows_burst\n");
        ...
        if (user_param->duplex) {
            fprintf(stderr,
                    " Flows is currently designed to work with unidir tests only\n");
            exit(FAILURE);
        }
    } else {
        if (user_param->flows_burst > 1) {
            fprintf(stderr,
                    " Flows burst is designed to work with more then single flow\n");
            exit(FAILURE);
        }
    }
    return;
...
static void force_dependecies(struct perftest_parameters* user_param) {
    ...
    if (user_param->test_type == ITERATIONS) {
        ...
        if (user_param->connection_type == UD ||
            ...
            if (user_param->rx_depth == DEF_RX_SEND) {
                user_param->rx_depth = (user_param->iters < UC_MAX_RX)
                                       ? user_param->iters : UC_MAX_RX;
            }
        }
    }
    ...
    if (user_param->test_type == DURATION) {
		...
		*/
        ...
        if (user_param->test_method == RUN_ALL) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    "Duration mode currently doesn't support running on all sizes.\n");
            exit(1);
        }
    ...
    if (user_param->connection_type == RawEth) {
        ...
        if (user_param->test_method == RUN_ALL) {
            fprintf(stderr,
                    "Raw Ethernet tests do not support -a / --all flag.\n");
            exit(1);
        }

        if (user_param->use_rdma_cm == ON || user_param->work_rdma_cm == ON) {
            fprintf(stderr,
                    " RDMA CM isn't supported for Raw Ethernet tests\n");
            exit(1);
        }

        if (user_param->use_gid_user) {
            fprintf(stderr,
                    " GID index isn't supported for Raw Ethernet tests\n");
            exit(1);
        }

        if (user_param->mmap_file != NULL || user_param->mmap_offset) {
            fprintf(stderr,
                    " mmaped files aren't supported for Raw Ethernet tests\n");
            exit(1);
        }
        ...
        if (user_param->is_dest_mac == OFF && (user_param->tst == LAT ||
                                               (user_param->machine == CLIENT &&
                                                !user_param->raw_mcast))) {
            printf(RESULT_LINE);
            fprintf(stderr,
        ...
        if ((user_param->is_server_port == ON &&
             user_param->is_client_port == OFF) ||
            (user_param->is_server_port == OFF &&
        ...
        if ((user_param->is_server_port == ON) &&
            (user_param->is_server_ip == OFF ||
             user_param->is_client_ip == OFF)) {
            printf(RESULT_LINE);
            fprintf(stderr,
        ...
        if (user_param->mac_fwd == ON &&
            user_param->cq_mod >= user_param->rx_depth) {
            fprintf(stderr, " CQ moderation can't be grater than rx depth.\n");
            user_param->cq_mod = user_param->rx_depth < user_param->tx_depth
                                 ? user_param->rx_depth : user_param->tx_depth;
            fprintf(stderr,
                    " Changing CQ moderation to min( rx depth , tx depth) = %d.\n",
                    user_param->cq_mod);
        }

        if (user_param->raw_mcast && user_param->duplex) {
            fprintf(stderr,
                    " Multicast feature works on unidirectional traffic only\n");
            exit(1);
        }
    ...
    if (user_param->work_rdma_cm) {
        ...
        fprintf(stdout,
                " TOS only valid for rdma_cm based QP and RawEth QP \n");
        exit(1);
    }

    if (user_param->hop_limit != DEF_HOP_LIMIT &&
        user_param->connection_type != RawEth) {
    ...
    if (user_param->test_method == RUN_INFINITELY) {
        ...
        if (user_param->use_event) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    " run_infinitely does not support events feature yet.\n");
            exit(1);
        }
        ...
        if (user_param->duplex && user_param->verb == SEND) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    " run_infinitely mode is not supported in SEND Bidirectional BW test\n");
            exit(1);
        }
        if (user_param->rate_limit_type != DISABLE_RATE_LIMIT) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    " run_infinitely does not support rate limit feature yet\n");
            exit(1);
        }
    }
    ...
    if ((user_param->use_srq &&
         (user_param->tst == LAT || user_param->machine == SERVER ||
          user_param->duplex == ON)) || user_param->use_xrc) {
    ...
    if (user_param->burst_size > 0) {
        if (user_param->rate_limit_type == DISABLE_RATE_LIMIT &&
            ...
            printf(RESULT_LINE);
            fprintf(stderr,
                    " Can't enable burst mode when rate limiter is off\n");
            exit(1);
        }
    }
    ...
    if (user_param->rate_limit_type == SW_RATE_LIMIT) {
        if (user_param->tst != BW || user_param->verb == ATOMIC ||
            (user_param->verb == SEND && user_param->duplex)) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    "SW Rate limiter cann't be executed on non-BW, ATOMIC or bidirectional SEND tests\n");
            exit(1);
        }
    } else if (user_param->rate_limit_type == HW_RATE_LIMIT) {
        if (user_param->use_rdma_cm == ON || user_param->work_rdma_cm == ON) {
            fprintf(stderr,
                    " HW rate limit isn't supported yet with rdma_cm scenarios\n");
            exit(1);
        }
        double rate_limit_gbps = 0;
        switch (user_param->rate_units) {
            case MEGA_BYTE_PS:
                rate_limit_gbps = ((double) (
                        ((user_param->rate_limit) * 8 * 1024 * 1024) /
                        1000000000));
                break;
            case GIGA_BIT_PS:
            ...
            case PACKET_PS:
                printf(RESULT_LINE);
                fprintf(stderr,
                        " Failed: pps rate limit units is not supported when setting HW rate limit\n");
                exit(1);
            default:
                printf(RESULT_LINE);
        ...
        if (rate_limit_gbps > 0) {
            ...
            get_gbps_str_by_ibv_rate(user_param->rate_limit_str,
                    &rate_index_to_set);
        ...
        if (user_param->rate_limit < 0) {
            fprintf(stderr,
                    " Must specify a rate limit when using Packet Pacing.\n Please add --rate_limit=<limit>.\n");
            exit(1);
        }
        if (user_param->connection_type != RawEth) {
            fprintf(stderr,
                    "Packet Pacing is only supported for Raw Ethernet.\n");
            exit(1);
        }
    ...
    if (user_param->tst == LAT_BY_BW) {
        if (user_param->test_type == DURATION) {
            fprintf(stderr,
                    "Latency under load test is currently support iteration mode only.\n");
            exit(1);
        }
        if (user_param->num_of_qps > 1) {
            fprintf(stderr,
                    "Multi QP is not supported in LAT under load test\n");
            exit(1);
        }
        if (user_param->duplex) {
    ...
    if (user_param->output != FULL_VERBOSITY) {
        if (user_param->tst == BW && !(user_param->output == OUTPUT_BW ||
                                       user_param->output == OUTPUT_MR)) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    " Output verbosity level for BW can be: bandwidth, message_rate\n");
            exit(1);
        }
        ...
        if (user_param->tst == LAT && !(user_param->output == OUTPUT_LAT)) {
            printf(RESULT_LINE);
            fprintf(stderr,
                    " Output verbosity level for LAT can be: latency\n");
            exit(1);
        }
    }
    ...
    if (user_param->report_per_port &&
        (user_param->test_type != DURATION || !user_param->dualport)) {
        printf(RESULT_LINE);
        fprintf(stderr,
                "report per port feature work only with Duration and Dualport\n");
        exit(1);
    }
    ...
    if (user_param->test_type == ITERATIONS && user_param->iters > 20000 &&
        user_param->noPeak == OFF && user_param->tst == BW) {
            user_param->noPeak = ON;
    }
 ...
 ******************************************************************************/
enum ibv_mtu
set_mtu(struct ibv_context* context, uint8_t ib_port, int user_mtu) {
    ...
    if (user_mtu == 0) {
        ...
        switch (user_mtu) {
            ...
            default   :
                fprintf(stderr, " Invalid MTU - %d \n", user_mtu);
                fprintf(stderr,
                        " Please choose mtu from {256,512,1024,2048,4096}\n");
    ...
    if (user_param->inline_size == DEF_INLINE) {
        ...
        if (user_param->tst == LAT) {
            ...
            switch (user_param->verb) {
                ...
                case WRITE:
                    user_param->inline_size = (user_param->connection_type ==
                                               DC) ? DEF_INLINE_DC
                                                   : DEF_INLINE_WRITE;
                    break;
                case SEND :
                    user_param->inline_size = (user_param->connection_type ==
                                               DC) ? DEF_INLINE_DC
                                                   : (user_param->connection_type ==
                                                      UD) ? DEF_INLINE_SEND_UD :
                                                     ((user_param->connection_type ==
                                                       XRC)
                                                      ? DEF_INLINE_SEND_XRC
                                                      : DEF_INLINE_SEND_RC_UC);
                    break;
                default   :
                    user_param->inline_size = 0;
            }
...
void set_raw_eth_parameters(struct perftest_parameters* user_param) {
    ...
    if (user_param->is_new_raw_eth_param == 1 &&
        user_param->is_old_raw_eth_param == 1) {
        printf(RESULT_LINE);
        fprintf(stderr,
                " Invalid Command line.\nMix of source with local|remote and dest with local|remote is not supported.\n");
        fprintf(stderr,
                "For L2 tests you must enter local and remote mac  by this format --local_mac XX:XX:XX:XX:XX:XX --remote_mac XX:XX:XX:XX:XX:XX\n");
        fprintf(stderr,
                "For L3 tests You must add also local and remote ip  by this format --local_ip X.X.X.X --remote_ip X.X.X.X\n");
        fprintf(stderr,
                "For L4 you need to add also local and remote port  by this format --local_port XXXX  --remote_port XXXX\n");
        exit(1);
    }
...
int parser(struct perftest_parameters* user_param, char* argv[], int argc) {
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv,
                "w:y:p:d:i:m:s:n:t:u:S:x:c:q:I:o:M:r:Q:A:l:D:f:B:T:L:E:J:j:K:k:X:aFegzRvhbNVCHUOZP",
                long_options, NULL);
        ...
        switch (c) {
            ...
            case 'p':
                ...
                break;
            case 'd': GET_STRING(user_param->ib_devname, strdupa(optarg));
                break;
            case 'i': CHECK_VALUE(user_param->ib_port, uint8_t, MIN_IB_PORT,
                    MAX_IB_PORT, "IB Port");
                break;
            case 'm':
                user_param->mtu = strtol(optarg, NULL, 0);
                break;
            case 'n': CHECK_VALUE(user_param->iters, int, MIN_ITER, MAX_ITER,
                    "Iteration num");
                break;
            case 't': CHECK_VALUE(user_param->tx_depth, int, MIN_TX, MAX_TX,
                    "Tx depth");
                break;
            case 'T': CHECK_VALUE(user_param->tos, int, MIN_TOS, MAX_TOS,
                    "TOS");
                break;
            case 'L': CHECK_VALUE(user_param->hop_limit, int, MIN_HOP_LIMIT,
                    MAX_HOP_LIMIT, "Hop Limit");
                break;
            case 'u':
                user_param->qp_timeout = (uint8_t) strtol(optarg, NULL, 0);
                break;
            case 'S':
                user_param->sl = (uint8_t) strtol(optarg, NULL, 0);
                ...
                break;
            case 'x': CHECK_VALUE(user_param->gid_index, uint8_t, MIN_GID_IX,
                    MAX_GID_IX, "Gid index");
                user_param->use_gid_user = 1;
                break;
            case 'c':
                change_conn_type(&user_param->connection_type, user_param->verb,
                        optarg);
                break;
            case 'q':
                if (user_param->tst != BW) {
                ...
                CHECK_VALUE(user_param->num_of_qps, int, MIN_QP_NUM, MAX_QP_NUM,
                        "num of Qps");
                break;
            case 'I': CHECK_VALUE(user_param->inline_size, int, 0, MAX_INLINE,
                    "Max inline");
                if (user_param->verb == READ || user_param->verb == ATOMIC) {
                    fprintf(stderr,
                            " Inline feature not available on READ/Atomic verbs\n");
                    return 1;
                }
                break;
            case 'o':
                ...
                if (user_param->verb != READ && user_param->verb != ATOMIC) {
                    fprintf(stderr,
                            " Setting Outstanding reads only available on READ verb\n");
                    return 1;
                }
                break;
            case 'M': GET_STRING(user_param->user_mgid, strdupa(optarg));
                break;
            case 'r': CHECK_VALUE(user_param->rx_depth, int, MIN_RX, MAX_RX,
                    " Rx depth");
                if (user_param->verb != SEND &&
                ...
                break;
            case 'Q': CHECK_VALUE(user_param->cq_mod, int, MIN_CQ_MOD,
                    MAX_CQ_MOD, "CQ moderation");
                break;
            case 'A':
                if (user_param->verb != ATOMIC) {
                    fprintf(stderr,
                            " You are not running the atomic_lat/bw test!\n");
                    fprintf(stderr,
                            " To change the atomic action type, you must run one of the atomic tests\n");
                    return 1;
                }
                ...
                if (strcmp(atomicTypesStr[0], optarg) == 0) {
                    ...
                    fprintf(stderr,
                            " Invalid Atomic type! please choose from {CMP_AND_SWAP,FETCH_AND_ADD}\n");
                    exit(1);
                }
                break;
            case 'l':
                user_param->post_list = strtol(optarg, NULL, 0);
                break;
            case 'D':
                user_param->duration = strtol(optarg, NULL, 0);
                if (user_param->duration <= 0) {
                    fprintf(stderr,
                            " Duration period must be greater than 0\n");
                    return 1;
                }
                user_param->test_type = DURATION;
                break;
            case 'f':
                user_param->margin = strtol(optarg, NULL, 0);
            ...
            case 's':
                ...
                user_param->size =
                        (uint64_t) strtol(optarg, NULL, 0) * size_factor;
                if (user_param->size<1 || user_param->size>(UINT_MAX / 2)) {
                    fprintf(stderr,
                            " Message Size should be between %d and %d\n", 1,
                            UINT_MAX / 2);
                    return 1;
                }
                break;
            ...
            case 'X':
                ...
                CHECK_VALUE(user_param->eq_num, int, MIN_EQ_NUM, MAX_EQ_NUM,
                        "EQN");
                break;
            case 'b':
                user_param->duplex = ON;
                if (user_param->tst == LAT) {
                    fprintf(stderr,
                            " Bidirectional is only available in BW test\n");
                    return 1;
                }
                break;
            case 'N':
            ...
            case 'C':
                ...
                break;
            case 'g':
                user_param->use_mcg = ON;
                if (user_param->verb != SEND) {
                    fprintf(stderr,
                            " MultiCast feature only available on SEND verb\n");
                    return 1;
                }
                break;
            case 'H':
            ...
            case 'Z':
                ...
                break;
            case 'v':
                user_param->mac_fwd = ON;
                break;
            case 'G':
                user_param->use_rss = ON;
                break;
            case 0: /* required for long options to work. */
                ...
                if (typical_pkt_size_flag) {
                    ...
                    if ((user_param->typical_pkt_size < 0) ||
                        ...
                        fprintf(stderr,
                                " Typical pkt size must be non-negative and less than 0xffff\n");
                        return FAILURE;
                    }
                ...
                if (rate_units_flag) {
                    if (strcmp("M", optarg) == 0) {
                        ...
                        fprintf(stderr,
                                " Invalid rate limit units. Please use M,g or p\n");
                        return FAILURE;
                    }
                    rate_units_flag = 0;
                }
                if (rate_limit_type_flag) {
                    ...
                    if (strcmp("SW", optarg) == 0) {
                        ...
                        fprintf(stderr,
                                " Invalid rate limit type flag. Please use HW, SW or PP.\n");
                        return FAILURE;
                    }
                ...
                if (verbosity_output_flag) {
                    if (strcmp("bandwidth", optarg) == 0) {
                        ...
                        fprintf(stderr,
                                " Invalid verbosity level output flag. Please use bandwidth, latency, message_rate\n");
                        return FAILURE;
                    }
                ...
                if (latency_gap_flag) {
                    ...
                    if (user_param->latency_gap < 0) {
                        fprintf(stderr,
                                " Latency gap time must be non-negative\n");
                        return FAILURE;
                    }
                    latency_gap_flag = 0;
                }
                ...
                if (retry_count_flag) {
                    ...
                    if (user_param->retry_count < 0) {
                        fprintf(stderr,
                                " Retry Count value must be positive\n");
                        return FAILURE;
                    }
                    retry_count_flag = 0;
                }
                if (verb_type_flag) {
                    if (strcmp("normal", optarg) == 0) {
                        ...
                        fprintf(stderr,
                                " Invalid verb type. Please choose normal/accl.\n");
                        return FAILURE;
                    }
                    verb_type_flag = 0;
                }
                ...
                if (tclass_flag) {
                    user_param->traffic_class = (uint16_t) strtol(optarg, NULL,
                            0);
                    tclass_flag = 0;
                }
                if (wait_destroy_flag) {
                    user_param->wait_destroy = (uint32_t) strtol(optarg, NULL,
                            0);
                    wait_destroy_flag = 0;
                }
                if (flows_flag) {
                    ...
                    if (user_param->flows == 0) {
                        fprintf(stderr,
                                "Invalid flows value. Please set a positive number\n");
                        return FAILURE;
                    }
                    flows_flag = 0;
                }
                if (flows_burst_flag) {
                    user_param->flows_burst = (uint16_t) strtol(optarg, NULL,
                            0);
                    if (user_param->flows_burst == 0) {
                        fprintf(stderr,
                                "Invalid burst flow value. Please set a positive number\n");
                        return FAILURE;
                    }
                    flows_burst_flag = 0;
                ...
                if (force_link_flag) {
                    ...
                    if (user_param->link_type == LINK_FAILURE) {
                        fprintf(stderr,
                                "Invalid link layer value should be IB or ETHERNET.\n");
                        return FAILURE;
                    }
                    force_link_flag = 0;
                }
                ...
                if (remote_port_flag) {
                    ...
                    if (OFF ==
                        check_if_valid_udp_port(user_param->remote_port)) {
                ...
                if (local_port_flag) {
                    ...
                    if (OFF ==
                        check_if_valid_udp_port(user_param->local_port)) {
                ...
                if (vlan_pcp_flag) {
                    ...
                    if (user_param->vlan_pcp > 8) {
                        fprintf(stderr,
                                "Invalid vlan priority value. Please set a number in 0~8\n");
                        return FAILURE;
                    }
                    vlan_pcp_flag = 0;
                }
 ...
 *
 ******************************************************************************/
int check_link_and_mtu(struct ibv_context* context,
        struct perftest_parameters* user_param) {
    ...
    if (user_param->connection_type == RawEth) {
        ...
        if (user_param->link_type != IBV_LINK_LAYER_ETHERNET) {
            fprintf(stderr,
                    " Raw Etherent test can only run on Ethernet link! exiting ...\n");
            return FAILURE;
        }
        ...
        user_param->curr_mtu = set_mtu(context, user_param->ib_port,
                user_param->mtu);
    }

    if (user_param->dualport == ON) {

        if (user_param->link_type2 == IBV_LINK_LAYER_ETHERNET &&
    ...
    if (user_param->verb == READ || user_param->verb == ATOMIC) {
        user_param->out_reads = ctx_set_out_reads(context,
    ...
    if (!user_param->ib_devname) GET_STRING(user_param->ib_devname,
...
void ctx_print_test_info(struct perftest_parameters* user_param) {
    ...
    printf(" Mtu             : %lu[B]\n",
            user_param->connection_type == RawEth ? user_param->curr_mtu
                                                  : MTU_SIZE(
                    user_param->curr_mtu));
...
static float calc_cpu_util(struct perftest_parameters* user_param) {
    ...
    ustat_diff = user_param->cpu_util_data.ustat[1] -
                 user_param->cpu_util_data.ustat[0];
    idle_diff = user_param->cpu_util_data.idle[1] -
                user_param->cpu_util_data.idle[0];
...
void print_report_bw(struct perftest_parameters* user_param,
    ...
    opt_delta = user_param->tcompleted[opt_posted] -
                user_param->tposted[opt_completed];
    ...
    if (user_param->noPeak == OFF) {
        ...
        for (i = 0; i < num_of_calculated_iters * num_of_qps; ++i) {
            for (j = i; j < num_of_calculated_iters * num_of_qps; ++j) {
                t = (user_param->tcompleted[j] - user_param->tposted[i]) /
                    (j - i + 1);
                if (t < opt_delta) {
                    opt_delta = t;
                    opt_posted = i;
    ...
    run_inf_bi_factor = (user_param->duplex &&
                         user_param->test_method == RUN_INFINITELY) ? (
                                user_param->verb == SEND ? 1 : 2) : 1;
    tsize = run_inf_bi_factor * user_param->size;
    num_of_calculated_iters *= (user_param->test_type == DURATION) ? 1
                                                                   : num_of_qps;
    ...
    sum_of_test_cycles = ((double) (user_param->tcompleted[location_arr] -
                                    user_param->tposted[0]));

    double bw_avg =
            ((double) tsize * num_of_calculated_iters * cycles_to_units) /
            (sum_of_test_cycles * format_factor);
    double msgRate_avg = ((double) num_of_calculated_iters * cycles_to_units *
                          run_inf_bi_factor) / (sum_of_test_cycles * 1000000);

    double bw_avg_p1 =
            ((double) tsize * user_param->iters_per_port[0] * cycles_to_units) /
            (sum_of_test_cycles * format_factor);
    double msgRate_avg_p1 =
            ((double) user_param->iters_per_port[0] * cycles_to_units *
             run_inf_bi_factor) / (sum_of_test_cycles * 1000000);

    double bw_avg_p2 =
            ((double) tsize * user_param->iters_per_port[1] * cycles_to_units) /
            (sum_of_test_cycles * format_factor);
    double msgRate_avg_p2 =
            ((double) user_param->iters_per_port[1] * cycles_to_units *
             run_inf_bi_factor) / (sum_of_test_cycles * 1000000);

    peak_up = !(user_param->noPeak) * (cycles_t) tsize *
    ...
    if (!user_param->duplex ||
        (user_param->verb == SEND && user_param->test_type == DURATION)
        || user_param->test_method == RUN_INFINITELY ||
        user_param->connection_type == RawEth) {
...
void print_full_bw_report(struct perftest_parameters* user_param,
    ...
    if ((user_param->duplex && rem_bw_rep != NULL) ||
        (!user_param->duplex && rem_bw_rep == NULL)) {
        /* Verify Limits */
        if (((user_param->is_limit_bw == ON) &&
             (user_param->limit_bw > bw_avg))) {
                 user_param->is_bw_limit_passed |= 0;
        } else {
                 user_param->is_bw_limit_passed |= 1;
        }

        if ((user_param->is_limit_msgrate) &&
            (user_param->limit_msgrate > msgRate_avg)) {
    ...
    if (user_param->output == OUTPUT_BW) {
        ...
        printf(REPORT_FMT_QOS, my_bw_rep->size, my_bw_rep->sl, my_bw_rep->iters,
                bw_peak, bw_avg, msgRate_avg);
    } else if (user_param->report_per_port) {
        printf(REPORT_FMT_PER_PORT, my_bw_rep->size, my_bw_rep->iters, bw_peak,
                bw_avg, msgRate_avg, bw_avg_p1, msgRate_avg_p1, bw_avg_p2,
                msgRate_avg_p2);
    } else {
        printf(inc_accuracy ? REPORT_FMT_EXT : REPORT_FMT, my_bw_rep->size,
                my_bw_rep->iters, bw_peak, bw_avg, msgRate_avg);
    }
    if (user_param->output == FULL_VERBOSITY) {
        fflush(stdout);
        fprintf(stdout, user_param->cpu_util_data.enable ? REPORT_EXT_CPU_UTIL
                                                         : REPORT_EXT,
...
void print_report_lat(struct perftest_parameters* user_param) {
    ...
    measure_cnt = (user_param->tst == LAT) ? user_param->iters - 1 :
                  (user_param->iters) / user_param->reply_every;
    rtt_factor = (user_param->verb == READ || user_param->verb == ATOMIC) ? 1
                                                                          : 2;
    ...
    if (user_param->tst == LAT) {
        ...
        fprintf(stderr,
                "print report LAT is support in LAT and LAT_BY_BW tests only\n");
        exit(1);
    }
...
void print_report_lat_duration(struct perftest_parameters* user_param) {
    ...
    latency = (((test_sample_time / cycles_to_units) / rtt_factor) /
               user_param->iters);
...
void print_report_fs_rate(struct perftest_parameters* user_param) {
    ...
    if (user_param->test_type == ITERATIONS) {
        ...
        average = latency;
        fps = user_param->iters /
              (test_sample_time / (cycles_to_units * units_to_sec));
    }

    if (user_param->output == FULL_VERBOSITY) {
        printf(RESULT_LINE);
        printf("%s", (user_param->test_type == ITERATIONS) ? RESULT_FMT_FS_RATE
                                                           : RESULT_FMT_FS_RATE_DUR);
        printf((user_param->cpu_util_data.enable ? RESULT_EXT_CPU_UTIL
                                                 : RESULT_EXT));
    }
    ...
    if (user_param->output == OUTPUT_LAT) {
        ...
        if (user_param->test_type == ITERATIONS) {
            fps = measure_cnt /
                  (average_sum / (cycles_to_units * units_to_sec));
