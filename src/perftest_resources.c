...
static enum ibv_wr_opcode opcode_verbs_array[] = {IBV_WR_SEND,
                                                  IBV_WR_RDMA_WRITE,
                                                  IBV_WR_RDMA_READ};
static enum ibv_wr_opcode opcode_atomic_array[] = {IBV_WR_ATOMIC_CMP_AND_SWP,
...
static int
get_n_word_string(char* input, char* output, int from_index, int iters) {
    for (; iters > 0; iters--) {
...
static void
get_cpu_stats(struct perftest_parameters* duration_param, int stat_index) {
...
int create_rdma_resources(struct pingpong_context* ctx,
    ...
    struct rdma_cm_id** cm_id = (user_param->machine == CLIENT) ? &ctx->cm_id
                                                                : &ctx->cm_id_control;
...
void alloc_ctx(struct pingpong_context* ctx,
    ...
    tarr_size = (user_param->noPeak) ? 1 : user_param->iters *
                                           user_param->num_of_qps;
    ALLOCATE(user_param->tposted, cycles_t, tarr_size);
    memset(user_param->tposted, 0, sizeof(cycles_t) * tarr_size);
    if ((user_param->tst == LAT || user_param->tst == FS_RATE) &&
        user_param->test_type == DURATION) ALLOCATE(user_param->tcompleted,
            cycles_t, 1);
	...
	* with reference to number of flows and number of QPs */
    ctx->buff_size = INC(BUFF_SIZE(ctx->size, ctx->cycle_buffer),
                             ctx->cache_line_size) * 2 * num_of_qps_factor *
                     user_param->flows;
    ...
    if (user_param->mmap_file != NULL) {
        ...
        if (ctx->is_contig_supported == FAILURE) {
            ...
            if (user_param->use_hugepages) {
                ...
                ctx->buf[qp_index] = memalign(user_param->cycle_buffer,
                        ctx->buff_size);
            }
#endif
            if (!ctx->buf[qp_index]) {
...
#else
    ctx->mr[qp_index] = ibv_reg_mr(ctx->pd, ctx->buf[qp_index], ctx->buff_size,
            flags);
#endif
...
int create_mr(struct pingpong_context* ctx,
    ...
    for (i = 1; i < user_param->num_of_qps; i++) {
        if (user_param->mr_per_qp) {
            ...
            ctx->mr[i] = ctx->mr[0];
            ctx->buf[i] =
                    ctx->buf[0] + (i * BUFF_SIZE(ctx->size, ctx->cycle_buffer));
        }
    }
...
int alloc_hugepage_region(struct pingpong_context* ctx, int qp_index) {
    ...
    ctx->buf[qp_index] = (void*) shmat(ctx->huge_shmid, SHMAT_ADDR,
            SHMAT_FLAGS);
    if (ctx->buf == (void*) -1) {
...
static int ctx_modify_qp_to_rtr(struct ibv_qp* qp,
    ...
    if (user_param->connection_type != RawEth) {
        ...
        if (((attr->ah_attr.port_num == user_param->ib_port) &&
            ...
            attr->ah_attr.is_global = 1;
            attr->ah_attr.grh.dgid = dest->gid;
            attr->ah_attr.grh.sgid_index = (attr->ah_attr.port_num ==
                                            user_param->ib_port)
...
void ctx_set_send_reg_wqes(struct pingpong_context* ctx,
    ...
    for (i = 0; i < num_of_qps; i++) {
        ...
        if (user_param->mac_fwd) {
            if (user_param->mr_per_qp) {
                ctx->sge_list[i * user_param->post_list].addr =
                        (uintptr_t) ctx->buf[0] + (num_of_qps + i) *
                                                  BUFF_SIZE(ctx->size,
                                                          ctx->cycle_buffer);
            } else {
                ctx->sge_list[i *
                              user_param->post_list].addr = (uintptr_t) ctx->buf[i];
            }
        }

        if (user_param->verb == WRITE || user_param->verb == READ) {
            ctx->wr[i * user_param->post_list].wr.rdma.remote_addr = rem_dest[
                    xrc_offset + i].vaddr;
        } else if (user_param->verb == ATOMIC) {
            ctx->wr[i * user_param->post_list].wr.atomic.remote_addr = rem_dest[
                    xrc_offset + i].vaddr;
        }
        ...
        for (j = 0; j < user_param->post_list; j++) {
            ...
            ctx->sge_list[i * user_param->post_list +
                          j].lkey = ctx->mr[i]->lkey;

            if (j > 0) {

                ctx->sge_list[i * user_param->post_list +
                              j].addr = ctx->sge_list[
                        i * user_param->post_list + (j - 1)].addr;
                ...
                if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) &&
                    user_param->size <= (ctx->cycle_buffer / 2)) {
                        increase_loc_addr(
                                ...
                                user_param->size,
                                j - 1, ctx->my_addr[i], 0, ctx->cache_line_size,
                                ctx->cycle_buffer);
                }
            }

            ctx->wr[i * user_param->post_list + j].sg_list = &ctx->sge_list[
                    i * user_param->post_list + j];
            ctx->wr[i * user_param->post_list + j].num_sge = MAX_SEND_SGE;
            ctx->wr[i * user_param->post_list + j].wr_id = i;

            if (j == (user_param->post_list - 1)) {
                ctx->wr[i * user_param->post_list +
                        j].send_flags = IBV_SEND_SIGNALED;
                ctx->wr[i * user_param->post_list + j].next = NULL;
            } else {
                ctx->wr[i * user_param->post_list + j].next = &ctx->wr[
                        i * user_param->post_list + j + 1];
                ctx->wr[i * user_param->post_list + j].send_flags = 0;
            }

            if (user_param->verb == ATOMIC) {
                ctx->wr[i * user_param->post_list +
                        j].opcode = opcode_atomic_array[user_param->atomicType];
            } else {
                ctx->wr[i * user_param->post_list +
                        j].opcode = opcode_verbs_array[user_param->verb];
            }
            if (user_param->verb == WRITE || user_param->verb == READ) {

                ctx->wr[i * user_param->post_list + j].wr.rdma.rkey = rem_dest[
                        xrc_offset + i].rkey;
                ...
                ctx->wr[i * user_param->post_list +
                        j].wr.atomic.rkey = rem_dest[xrc_offset + i].rkey;

                if (j > 0) {

                    ctx->wr[i * user_param->post_list +
                            j].wr.atomic.remote_addr =
                            ctx->wr[i * user_param->post_list + j -
                ...
                if (user_param->atomicType == FETCH_AND_ADD) {
                    ctx->wr[i * user_param->post_list +
                            j].wr.atomic.compare_add = ATOMIC_ADD_VALUE;
                } else {
                    ctx->wr[i * user_param->post_list +
                            j].wr.atomic.swap = ATOMIC_SWAP_VALUE;
                }
                ...
                if (user_param->connection_type == UD) {
                    ...
                    ctx->wr[i * user_param->post_list +
                            j].wr.ud.remote_qkey = remote_qkey;
                    ctx->wr[i * user_param->post_list +
                            j].wr.ud.remote_qpn = remote_qpn;
                }
            }

            if ((user_param->verb == SEND || user_param->verb == WRITE) &&
                user_param->size <= user_param->inline_size) {
                    ctx->wr[i * user_param->post_list +
                            j].send_flags |= IBV_SEND_INLINE;
            }
...
int ctx_set_recv_wqes(struct pingpong_context* ctx,
    ...
    for (k = 0; i < user_param->num_of_qps; i++, k++) {
        ...
        ctx->recv_sge_list[i].length = SIZE(user_param->connection_type,
                user_param->size, 1);
        ...
        for (j = 0; j < size_per_qp; ++j) {
            ...
            if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) &&
                ...
                increase_loc_addr(&ctx->recv_sge_list[i],
                        ...
                        user_param->connection_type, ctx->cache_line_size,
                        ctx->cycle_buffer);
            }
        }
...
int run_iter_bw(struct pingpong_context* ctx,
    ...
    if (user_param->rate_limit_type == SW_RATE_LIMIT) {
        ...
        switch (user_param->rate_units) {
            case MEGA_BYTE_PS:
                rate_limit_pps =
                        ((double) (user_param->rate_limit) / user_param->size) *
                        1048576;
                break;
            case GIGA_BIT_PS:
                rate_limit_pps = ((double) (user_param->rate_limit) /
                                  (user_param->size * 8)) * 1000000000;
                break;
            case PACKET_PS:
            ...
            while ((ctx->scnt[index] < user_param->iters ||
                ...
                if (user_param->flows != DEF_FLOWS) {
                    if (++flows_burst_iter == user_param->flows_burst) {
                        ...
                        ctx->sge_list[0].addr =
                                primary_send_addr + address_offset;
                    }
                }
                ...
                if (user_param->post_list == 1 &&
                    ...
                    increase_loc_addr(ctx->wr[index].sg_list, user_param->size,
                            ctx->scnt[index],
                            ctx->my_addr[index] + address_offset, 0,
                            ctx->cache_line_size,
                            ctx->cycle_buffer);
                    ...
                    if (user_param->verb != SEND) {
                        ...
                        increase_rem_addr(&ctx->wr[index], user_param->size,
                                ctx->scnt[index], ctx->rem_addr[index],
                                user_param->verb,
                                ctx->cache_line_size, ctx->cycle_buffer);
                    }
                }
        ...
        do {
            ...
            if (ne > 0) {
                ...
                for (i = 0; i < ne; i++) {
                    ...
                    if (user_param->test_type == DURATION ||
                        rcnt_for_qp[wc_id] + size_per_qp <= user_param->iters) {
#ifdef HAVE_ACCL_VERBS
                                                                                                                                                if (user_param->verb_type == ACCL_INTF) {
...
#endif
                        if (user_param->use_srq) {
                            if (ibv_post_srq_recv(ctx->srq, &ctx->rwr[wc_id],
                                    &bad_wr_recv)) {
                                fprintf(stderr,
                            ...
                            if (ibv_post_recv(ctx->qp[wc_id], &ctx->rwr[wc_id],
                                    &bad_wr_recv)) {
                                fprintf(stderr,
                        ...
                        if (user_param->flows != DEF_FLOWS) {
                            if (++recv_flows_burst == user_param->flows_burst) {
                                ...
                                address_flows_offset =
                                        recv_flows_index * ctx->cycle_buffer;
...
#endif
                        if (SIZE(user_param->connection_type, user_param->size,
                    ...
                    if (ctx->send_rcredit) {
                        int credit_cnt =
                                rcnt_for_qp[wc_id] % user_param->rx_depth;
                        ...
                        if (credit_cnt % ctx->credit_cnt == 0) {
                            ...
                            if (ibv_post_send(ctx->qp[wc_id],
                                    &ctx->ctrl_wr[wc_id], &bad_wr)) {
                                fprintf(stderr,
...
int run_iter_bi(struct pingpong_context* ctx,
    ...
    int size_per_qp = (user_param->use_srq) ? user_param->rx_depth /
                                              user_param->num_of_qps
                                            : user_param->rx_depth;
    int return_value = 0;
    ...
    while ((user_param->test_type == DURATION &&
        ...
        for (index = 0; index < num_of_qps; index++) {
            while (before_first_rx == OFF && (ctx->scnt[index] < iters ||
                ...
                if (user_param->post_list == 1 &&
                    ...
                    increase_loc_addr(ctx->wr[index].sg_list, user_param->size,
                            ctx->scnt[index],
                            ctx->my_addr[index], 0, ctx->cache_line_size,
                            ctx->cycle_buffer);
                }
                                ...
                                if (credit_wc.opcode == IBV_WC_RDMA_WRITE) {
                                    ...
                                    if (user_param->noPeak == OFF) {
                                        if ((user_param->test_type ==
                                             ITERATIONS &&
                                             (totccnt >= tot_iters - 1))) {
                                                 user_param->tcompleted[tot_iters -
                                                                        1] = get_cycles();
                                        } else {
                                                 user_param->tcompleted[totccnt -
                                                                        1] = get_cycles();
                                        }
                                    }
                                    if (user_param->test_type == DURATION &&
...
int run_iter_lat_write(struct pingpong_context* ctx,
    ...
    poll_buf = (char*) ctx->buf[0] +
               ...
               BUFF_SIZE(ctx->size, ctx->cycle_buffer) + user_param->size - 1;
        ...
        if ((rcnt < user_param->iters || user_param->test_type == DURATION) &&
            ...
            do {
                ...
                if (ne > 0) {
					 ...
					 */
                    if (user_param->test_type == DURATION ||
                        (rcnt + size_per_qp <= user_param->iters)) {
                        if (user_param->use_srq) {

                            if (ibv_post_srq_recv(ctx->srq, &ctx->rwr[wc.wr_id],
                                    &bad_wr_recv)) {
                                fprintf(stderr,
                                        "Couldn't post recv SRQ. QP = %d: counter=%lu\n",
                                        (int) wc.wr_id, rcnt);
                            ...
                            if (ibv_post_recv(ctx->qp[wc.wr_id],
                                    &ctx->rwr[wc.wr_id], &bad_wr_recv)) {
                                fprintf(stderr,
                        ...
                        if (user_param->flows != DEF_FLOWS) {
                            if (++recv_flows_index == user_param->flows) {
                                ...
                                ctx->recv_sge_list[0].addr += INC(
                                        user_param->size, ctx->cache_line_size);
                            }
                        }
                    }
                } else if (ne < 0) {
        ...
        while ((totscnt < user_param->iters)
            ...
            if (user_param->post_list == 1 &&
                ...
                increase_loc_addr(ctx->wr[0].sg_list, user_param->size, totscnt,
                        ctx->my_addr[0], 0, ctx->cache_line_size,
                        ctx->cycle_buffer);
            }
            totscnt += user_param->post_list;
...
void check_alive(int sig) {
    if (check_alive_data.current_totrcnt > check_alive_data.last_totrcnt) {
        ...
        alarm(60);
            ...
            check_alive_data.current_totrcnt == check_alive_data.last_totrcnt &&
...
int run_iter_fs(struct pingpong_context* ctx,
    ...
    if (user_param->test_type == ITERATIONS) {
        ...
        allocated_flows = (2 * MAX_FS_PORT) -
                          (user_param->server_port + user_param->client_port);
    }
...
#else
    ALLOCATE(flow_create_result, struct ibv_flow*,
            allocated_flows * user_param->num_of_qps);
    ALLOCATE(flow_rules, struct ibv_flow_attr*,
            allocated_flows * user_param->num_of_qps);
#endif
...
#else
                flow_create_result[flow_index] =
                        ibv_create_flow(ctx->qp[qp_index],
                                flow_rules[(qp_index * allocated_flows) +
                                           flow_index]);
#endif
                if (user_param->test_type == ITERATIONS) {
    ...
    for (i = 0; i < user_param->num_of_qps; i++) {
        ...
        if (user_param->machine == CLIENT) {
            rc = rdma_create_id(ctx->cma_master.channel,
                    &ctx->cma_master.nodes[i].cma_id, NULL,
                    hints->ai_port_space);
            if (rc) {
