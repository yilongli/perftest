...
static void signalCatcher(int sig) {
    if (sig == SIGINT) {
        ...
        if (sighandler_params->is_2nd_mgid_used) {
            memcpy(sighandler_params->mgid.raw,
                    sighandler_params->base_mgid.raw, 16);
            if (join_multicast_group(SUBN_ADM_METHOD_DELETE,
                    sighandler_params)) {
                        fprintf(stderr,
                                "Couldn't Unregister the Base Mcast group on the SM\n");
            }
        }
    }
    exit(1);
...
static void prepare_mcast_mad(uint8_t method,
    ...
    (*(uint64_t*) (ptr + 8)) = ntoh_64(
            (uint64_t) DEF_TRANS_ID);             /* TransactionID */
    (*(uint16_t*) (ptr + 16)) = htons(
            SUBN_ADM_ATTR_MC_MEMBER_RECORD);      /* AttributeID */
    ...
    comp_mask = SUBN_ADM_COMPMASK_MGID | SUBN_ADM_COMPMASK_PORT_GID |
                SUBN_ADM_COMPMASK_Q_KEY |
                SUBN_ADM_COMPMASK_P_KEY | SUBN_ADM_COMPMASK_TCLASS |
                SUBN_ADM_COMPMASK_SL |
 ...
 ******************************************************************************/
static void
get_mlid_from_mad(struct sa_mad_packet_t* samad_packet, uint16_t* mlid) {
    uint8_t* ptr;
    ptr = samad_packet->SubnetAdminData;
...
join_multicast_group(subn_adm_method method, struct mcast_parameters* params) {
    ...
    if (umad_set_addr(umad_buff, params->sm_lid, 1, params->sm_sl,
            QP1_WELL_KNOWN_Q_KEY) < 0) {
