...
static int create_ah_from_wc_recv(struct pingpong_context* ctx,
    ...
    ctx->ah[0] = ibv_create_ah_from_wc(ctx->pd, &wc,
            (struct ibv_grh*) ctx->buf[0], ctx->cm_id->port_num);
...
static int get_best_gid_index(struct pingpong_context* ctx,
    ...
    for (i = 1; i < attr->gid_tbl_len; i++) {
        ...
        is_ipv4_rival = ipv6_addr_v4mapped(
                (struct in6_addr*) temp_gid_rival.raw);
    ...
    */
    ...
    if (user_param->gid_index != -1) {
        ...
        if (user_param->use_gid_user) {
            if (ibv_query_gid(ctx->context, user_param->ib_port,
                    user_param->gid_index, &temp_gid)) {
                        return -1;
            }
        } else {
            user_param->gid_index = get_best_gid_index(ctx, user_param, &attr,
                    user_param->ib_port);
    ...
    if (user_param->dualport == ON) {
        if (user_param->gid_index2 != -1) {
            ...
            if (user_param->use_gid_user) {
                if (ibv_query_gid(ctx->context, user_param->ib_port2,
                        user_param->gid_index, &temp_gid2)) {
                            return -1;
                }
            } else {
                user_param->gid_index2 = get_best_gid_index(ctx, user_param,
                        &attr, user_param->ib_port2);
                if (user_param->gid_index2 < 0) {
                    return -1;
                }
                if (ibv_query_gid(ctx->context, user_param->ib_port2,
                        user_param->gid_index2, &temp_gid2)) {
                            return -1;
                }
            }
        }
        ...
        if (user_param->dualport == ON) {
            ...
            */
            if (i % num_of_qps < num_of_qps_per_port) {
                my_dest[i].lid = ctx_get_local_lid(ctx->context,
                        user_param->ib_port);
...
int rdma_client_connect(struct pingpong_context* ctx,
    ...
    if (check_add_port(&service, user_param->port, user_param->servername,
            &hints, &res)) {
        fprintf(stderr, "Problem in resolving basic address and port\n");
        return FAILURE;
    }
    ...
    while (1) {
        ...
        if (rdma_resolve_addr(ctx->cm_id, NULL, (struct sockaddr*) &sin,
                2000)) {
    ...
    if (user_param->tos != DEF_TOS) {
        ...
        if (rdma_set_option(ctx->cm_id, RDMA_OPTION_ID, RDMA_OPTION_ID_TOS,
                &user_param->tos, sizeof(uint8_t))) {
...
int create_comm_struct(struct perftest_comm* comm,
    ...
    if (user_param->use_rdma_cm) {
        ...
        ALLOCATE(comm->rdma_ctx->qp, struct ibv_qp*,
                comm->rdma_params->num_of_qps);
 ...
 ******************************************************************************/
int
ethernet_read_data(struct perftest_comm* comm, char* recv_msg, size_t size) {
...
xchg_bw_reports(struct perftest_comm* comm, struct bw_report_data* my_bw_rep,
    ...
    /*******************Exchange Reports*******************/
    if (ctx_xchg_data(comm, (void*) (&temp.size), (void*) (&rem_bw_rep->size),
    ...
    if (ctx_xchg_data(comm, (void*) (&temp.iters), (void*) (&rem_bw_rep->iters),
            size)) {
        fprintf(stderr,
                " Failed to exchange data between server and clients\n");
        exit(1);
    }
    if (ctx_xchg_data(comm, (void*) (&temp.bw_peak),
            (void*) (&rem_bw_rep->bw_peak), sizeof(double))) {
        fprintf(stderr,
                " Failed to exchange data between server and clients\n");
        exit(1);
    }
    if (ctx_xchg_data(comm, (void*) (&temp.bw_avg),
            (void*) (&rem_bw_rep->bw_avg), sizeof(double))) {
        fprintf(stderr,
                " Failed to exchange data between server and clients\n");
        exit(1);
    }
    if (ctx_xchg_data(comm, (void*) (&temp.msgRate_avg),
            (void*) (&rem_bw_rep->msgRate_avg), sizeof(double))) {
        fprintf(stderr,
                " Failed to exchange data between server and clients\n");
        exit(1);
    }
    ...
    if (comm->rdma_params->report_per_port) {
        if (ctx_xchg_data(comm, (void*) (&temp.bw_avg_p1),
                (void*) (&rem_bw_rep->bw_avg_p1), sizeof(double))) {
            fprintf(stderr,
                    " Failed to exchange data between server and clients\n");
            exit(1);
        }
        if (ctx_xchg_data(comm, (void*) (&temp.msgRate_avg_p1),
                (void*) (&rem_bw_rep->msgRate_avg_p1), sizeof(double))) {
            fprintf(stderr,
                    " Failed to exchange data between server and clients\n");
            exit(1);
        }
        if (ctx_xchg_data(comm, (void*) (&temp.bw_avg_p2),
                (void*) (&rem_bw_rep->bw_avg_p2), sizeof(double))) {
            fprintf(stderr,
                    " Failed to exchange data between server and clients\n");
            exit(1);
        }
        if (ctx_xchg_data(comm, (void*) (&temp.msgRate_avg_p2),
                (void*) (&rem_bw_rep->msgRate_avg_p2), sizeof(double))) {
            fprintf(stderr,
                    " Failed to exchange data between server and clients\n");
            exit(1);
        }
...
void ctx_print_pingpong_data(struct pingpong_dest* element,
    ...
    printf(BASIC_ADDR_FMT, sideArray[comm->rdma_params->side], dlid,
            element->qpn, element->psn);

    switch (comm->rdma_params->verb) {
        case 2  :
            printf(READ_FMT, element->out_reads);
        case 1  :
    ...
    local_mgid =
            (comm->rdma_params->side == 0) && (comm->rdma_params->machine == 0);
    remote_mgid =
            (comm->rdma_params->side == 1) && (comm->rdma_params->machine == 1);
    is_there_mgid = comm->rdma_params->duplex || remote_mgid || local_mgid;

    if ((comm->rdma_params->gid_index > -1 ||
         (comm->rdma_params->use_mcg && is_there_mgid)) &&
        comm->rdma_params->connection_type != RawEth) {

        printf(PERF_GID_FMT,
                gidArray[comm->rdma_params->use_mcg && is_there_mgid],
...
int ctx_close_connection(struct perftest_comm* comm,
    ...
    if (!comm->rdma_params->use_rdma_cm && !comm->rdma_params->work_rdma_cm) {
        ...
        if (write(comm->rdma_params->sockfd, "done", sizeof "done") !=
            sizeof "done") {
            perror(" Client write");
...
void check_sys_data(struct perftest_comm* user_comm,
    ...
    if (!user_param->dont_xchg_versions) {
        if (ctx_xchg_data(user_comm, (void*) (&m_cycle_buffer),
        ...
        if (ctx_xchg_data(user_comm, (void*) (&m_cache_line_size),
    ...
    user_param->cycle_buffer = (rem_cycle_buffer > user_param->cycle_buffer)
                               ? rem_cycle_buffer : user_param->cycle_buffer;
    user_param->cache_line_size = (rem_cache_line_size >
                                   user_param->cache_line_size)
                                  ? rem_cache_line_size
                                  : user_param->cache_line_size;
...
check_mtu(struct ibv_context* context, struct perftest_parameters* user_param,
    ...
    if (user_param->connection_type == RawEth) {
        ...
        curr_mtu = (int) (set_mtu(context, user_param->ib_port,
                user_param->mtu));
        if (!user_param->dont_xchg_versions) {
            ...
            if (rem_vers >= 5.1) {
                ...
                size_of_cur = (rem_vers >= 5.31) ? sizeof(char[2])
                                                 : sizeof(int);

                if (ctx_xchg_data(user_comm, (void*) (cur), (void*) (rem),
                        size_of_cur)) {
                    fprintf(stderr,
                            " Failed to exchange data between server and clients\n");
                    exit(1);
                ...
                user_param->curr_mtu = (enum ibv_mtu) ((valid_mtu_size(
                        rem_mtu) && (curr_mtu > rem_mtu)) ? rem_mtu : curr_mtu);
            } else {
